...
import { ChevronLeft, ChevronRight, AlertTriangle, Calendar as CalendarIcon, Clock, Sparkles, Plus, Loader2 } from "lucide-react";
import { getAIRecommendations, addActivityToCalendar } from "@/app/actions/calendar";
...
export default function CalendarSync() {
  ...
  useEffect(() => {
    async function loadRecommendations() {
      setIsLoading(true);
      const { success, data } = await getAIRecommendations();
      if (success && data) {
  ...
  const [calendarEvents, setCalendarEvents] = useState<any[]>(initialDummyEvents);

  const handleAddToCalendar = async (rec: any) => {
    // 1) DB에 저장 (Server Action 호출)
    const { success, error } = await addActivityToCalendar(rec.id);
    ...
    alert(`"${rec.title}" 활동이 확정 캘린더에 성공적으로 담겼습니다!`);

    // 2) UI 즉각 업데이트 (임시 -> 확정 상태로 변환)
    const sDay = rec.raw_start_date ? parseInt(rec.raw_start_date.split('-')[2]) : selectedDate;
    const eDay = rec.raw_end_date ? parseInt(rec.raw_end_date.split('-')[2]) : selectedDate;

    setCalendarEvents(prev => [...prev, {
      id: rec.id,
      startDay: sDay,
      endDay: eDay,
      date: eDay, // fallback for legacy logic
      type: "career",
      title: rec.title,
      time: "10:00",
      tag: rec.category,
      color: "bg-purple-100 text-purple-700 border-purple-300 shadow-sm" // 확정된 추천 활동의 스타일
    }]);
  };

  const getEventsForDate = (day: number) => {
    // 1. 기존 확정된 이벤트 목록
    const saved = calendarEvents.filter(e => {
      // 기간 이벤트인지 (startDay ~ endDay) 아니면 단일 이벤트인지 (date) 확인
    ...
    // 2. AI 추천 임시 데이터 (아직 확정배열에 들어오지 않은 id만)
    const savedIds = new Set(calendarEvents.filter(e => e.id).map(e => e.id));
    const temporary = recommendations
      .filter(rec => !savedIds.has(rec.id))
      .filter(rec => {
        const sDay = rec.raw_start_date ? parseInt(rec.raw_start_date.split('-')[2]) : 20;
        const eDay = rec.raw_end_date ? parseInt(rec.raw_end_date.split('-')[2]) : 25;
    ...
    <div className="flex flex-col gap-6 lg:gap-8 h-full animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 min-h-[650px] items-start">
        ...
        <div className="lg:col-span-2 glass-card p-6 md:p-8 flex flex-col h-full shadow-sm relative w-full">
            ...
            {days.map((date, i) => {
              ...
              return (
                <div
                  ...
                  className={`bg-white min-h-[110px] p-2 flex flex-col transition-colors border-[2px] ${isSelected ? "border-primary bg-primary/5 z-10 shadow-sm" : "border-transparent hover:bg-gray-50 cursor-pointer"
                    } ${!isValidDate ? "opacity-40 pointer-events-none" : ""}`}
                >
                  <div className="flex justify-between items-start mb-2">
                    <span className={`text-sm font-bold w-7 h-7 flex items-center justify-center rounded-full transition-colors ${isToday ? "bg-primary text-white shadow-md" :
                      ...
                      <div key={idx} className={`relative text-[10px] font-extrabold px-1.5 py-1 rounded truncate border group/ev ${ev.color}`}>
                        ...
                        {ev.isTemp && (
                          <div className="absolute inset-0 bg-purple-900/90 text-white flex items-center justify-center rounded opacity-0 group-hover/ev:opacity-100 transition-opacity z-50">
                            <button
                              onClick={(e) => { e.stopPropagation(); handleAddToCalendar(ev.rawRec); }}
                              className="text-[9px] font-black w-full h-full flex items-center justify-center"
                            >
                              확인 (저장)
                            </button>
                          </div>
          ...
          {/* AI Recommended Activities Section */}
          <div className="mt-8 border-t border-gray-100 pt-8 relative overflow-hidden rounded-2xl">
            <div className="flex items-center justify-between mb-6 relative z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-extrabold text-gray-900 text-lg tracking-tight">AI 맞춤 대외활동 추천</h3>
                  <p className="text-xs font-bold text-gray-500">유저님의 온보딩 프로필을 분석한 맞춤형 추천입니다.</p>
                </div>
              </div>
              <button className="text-xs font-extrabold text-purple-600 bg-purple-50 border border-purple-100 hover:bg-purple-100 px-4 py-2 rounded-xl transition-all">모두 보기</button>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 relative z-10">
              {isLoading ? (
                // 로딩 레이아웃
                <div className="col-span-3 flex flex-col items-center justify-center py-10 bg-white/50 rounded-xl border border-dashed border-gray-200">
                  <Loader2 className="w-8 h-8 text-purple-400 animate-spin mb-3" />
                  <p className="text-sm font-bold text-gray-400">AI가 유저님을 위한 대외활동을 분석 중입니다...</p>
                </div>
              ) : (
                recommendations.map(rec => (
                  <div key={rec.id} className="bg-white border-2 border-transparent shadow-[0_2px_10px_rgba(0,0,0,0.02)] rounded-3xl p-5 hover:border-purple-200 hover:shadow-xl hover:shadow-purple-500/10 transition-all duration-300 group flex flex-col cursor-pointer">
                    <div className="flex justify-between items-start mb-3 bg-white">
                      <span className="text-[10px] font-extrabold px-2 py-1 bg-gray-50 border border-gray-100 text-gray-600 rounded-lg">{rec.category}</span>
                      <span className="text-xs font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-indigo-500">{rec.tag}</span>
                    </div>
                    ...
                    <span className="flex items-center gap-1.5 text-[11px] font-bold text-gray-400 mb-4"><CalendarIcon className="w-3.5 h-3.5" /> 지원 마감: {rec.date}</span>
                    <div className="mt-auto">
                      <div className="text-[11px] bg-purple-50/80 p-3 rounded-xl text-purple-700 font-bold leading-relaxed mb-4 border border-purple-100/50">
                        💡 {rec.reason}
                      </div>
                      <button
                        onClick={() => handleAddToCalendar(rec)}
                        className="w-full py-2.5 bg-gray-900 text-white text-xs font-black rounded-xl hover:bg-primary hover:shadow-lg hover:shadow-primary/20 transition-all flex items-center justify-center gap-1.5 active:scale-95"
                      >
                        <Plus className="w-4 h-4" /> 캘린더에 쏙 담기
                      </button>
                    </div>
                  </div>
                ))
